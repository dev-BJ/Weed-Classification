...
if __name__ == '__main__':
    ...
    lbp = list(feature_generator(df['gray_path'], img_size=img_size))
    valid_lbp = [i for i, (hist, _) in enumerate(lbp) if hist is not None]

    # Feature extraction with error handling
    X = np.array([lbp[i][0] for i in valid_lbp])
    
    y = np.array([df['label'].values[i] for i in valid_lbp])
    # print(y.shape)

    # lbp = valid_lbp = hog = valid_hog = valid_indices = None
    ...
    try:
        os.makedirs(os.path.join(base_path, 'process/out/lbp'), exist_ok=True)
        joblib.dump(model, os.path.join(base_path, "process/out/lbp", "trained_weed.joblib"))
        joblib.dump(label_map, os.path.join(base_path, "process/out/lbp", "label_encoder.joblib"))
        joblib.dump(scaler, os.path.join(base_path, "process/out/lbp", "scaler.joblib"))
    ...
    plt.savefig(os.path.join(base_path, 'process/out/lbp', 'confusion_matrix.png'))
    plt.show()
    ...
    if sample_features[0] is not None:
        ...
        plt.savefig(os.path.join(base_path, 'process/out/lbp', 'lbp_histogram.png'))
        plt.show()
